...
- Cloud Run revision `secondkey-agent-00006-mx7` is public in `australia-southeast2` and serves 100% of traffic with max instances 1. Hosted `/status`, `/fleet`, `/registry`, `/audit.*`, `/triage` and `/fleet/run` are verified. The fleet returned HTTP 200 after seven Vertex provider calls and kept `external_write:false`; evidence is in `evidence/live-fleet-run-cloudrun.json`. The special `/healthz` path still returns a front-door 404. The prior revision's request-end flush produced two verified `contextops.audit.Intake___Triage` spans in Cloud Trace; evidence is in `evidence/cloud-trace-after-flush.json`.

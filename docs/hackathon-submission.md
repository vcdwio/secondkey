...
|---|---|---|
| Real Gemini workflow | Locally implemented and smoke-tested | `agent/scripts/smoke.ts`; queued mail must call `score_priority` |
...
| Telemetry | Local verified; Cloud exporter prepared | OTel spans plus JSON and formula-safe CSV |
| Hosted Cloud Run URL | Not yet created | requires explicit deployment authorization |
...
- Project name: **Verge AI - The Fortified Enterprise Fleet**
- One-sentence value: A governed fleet of enterprise operations agents that turns fragmented requests into evidence-backed decisions without giving the model authority over priority, permission, money, or staffing.
